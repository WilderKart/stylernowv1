...
### ADL-028 — `REVOKE ... FROM <rol>` no cierra una función mientras el `GRANT EXECUTE TO PUBLIC` implícito siga vivo — espejo exacto de ADL-022
**Fecha:** 2026-09-16
**Decisión:** Cualquier `revoke execute on function ...` que busque cerrar el acceso de un rol específico (`authenticated`, `anon`) debe revocar TAMBIÉN de `public` en la misma sentencia (`revoke execute on function f(...) from public, anon, authenticated;`) — nunca solo del rol que se quiere bloquear.
**Motivo:** Migración 073 (ADR-014, Fase B) intentó cerrar el autoservicio remoto de `redimir_gift_card()` con `revoke execute ... from authenticated;` — la propia suite de verificación de Fase B demostró que un Cliente TODAVÍA podía canjear su Gift Card a distancia después de aplicada esa migración. Causa: Postgres otorga `EXECUTE` a `PUBLIC` por defecto en toda función nueva (a diferencia de `SELECT`/`INSERT` en tablas, que si respetan RLS); revocar de un rol puntual no quita el acceso heredado de `PUBLIC`. Es la misma trampa que ADL-022 documentó para el sentido opuesto ("revocar de `public` no bloquea a `anon`/`authenticated`") — acá el error fue simétrico: revocar de un rol puntual no bloquea el acceso que sigue abierto vía `public`.
**Impacto:** Corregido en la migración 075. Regla permanente para este proyecto: toda sentencia `revoke execute` debe listar explícitamente `public, anon, authenticated` (el mismo patrón que `_lealtad_acreditar`/`otorgar_creditos_plan_mensual` ya usaban correctamente) — nunca un rol aislado. Verificado con una prueba real que intenta la llamada bloqueada y confirma `permission denied`, no solo que la migración corrió sin error.
**Estado:** Activa — corregido y verificado.

### ADL-029 — `create or replace function` con una lista de parámetros distinta crea un OVERLOAD nuevo, no reemplaza la función existente
**Fecha:** 2026-09-16
**Decisión:** Agregar parámetros nuevos (aunque tengan `default`) a una función ya existente requiere un `drop function if exists public.f(tipos_viejos...)` explícito en la misma migración — nunca asumir que `create or replace function` con una firma distinta reemplaza sola la versión anterior.
**Motivo:** Migración 073 extendió `crear_gift_card()` de 6 a 8 parámetros usando `create or replace function`. Postgres identifica una función por su firma completa (tipos y cantidad de parámetros, no el nombre solo) — al cambiar la cantidad de parámetros, la sentencia creó un SEGUNDO overload en vez de reemplazar el primero, dejando ambas versiones (6 y 8 parámetros) coexistiendo. Una llamada real con pocos argumentos (apoyándose en los `default` de ambas firmas) se volvió ambigua para PostgREST: "Could not choose the best candidate function" — descubierto por la propia suite de verificación de Fase B.
...
### ADL-031 — El Pipeline de Eventos deja de ser exclusivo del POS: se convierte en el bus oficial de eventos de StylerNow
**Fecha:** 2026-09-16
**Decisión:** `venta_pipeline_handler`/`ejecutar_pipeline_venta_completada()` (ADL-027, exclusivos de `venta_completada`) se generalizan a `evento_pipeline_handler` (columna `evento` nueva, con un catálogo cerrado vía `check` — no una tabla de referencia separada) y `ejecutar_pipeline_evento(p_evento_tipo, p_payload)`. Cualquier módulo futuro que necesite reaccionar a un evento de la plataforma se registra con una fila nueva en `evento_pipeline_handler` — nunca modificando de nuevo la función que emite el evento.
**Motivo:** Pedido explícito del fundador (ADR-014, Fase F, "obligatoria"): "el pipeline no debe quedar acoplado únicamente al POS... nunca volver a modificar directamente `completar_venta_pos()` para agregar módulos futuros." El catálogo oficial de eventos reconocidos incluye `venta_completada` (7 handlers reales), `reserva_confirmada`, `cliente_registrado`, `plan_actualizado` y `staff_trasladado` — estos últimos 4 se reconocen en el `check` constraint (listos para recibir un handler el día que exista un consumidor real) pero **deliberadamente sin ningún handler fabricado hoy**: `reserva_confirmada` no tiene más efecto real que actualizar `reserva.estado` (WhatsApp diferido, Agenda ya lee en vivo); `cliente_registrado` no tiene ningún efecto de Lealtad/Referidos por diseño (la recompensa de Referido se otorga en la primera Reserva completada, nunca en el registro — Módulo 4, ADR-011); `plan_actualizado` no debe recalcular créditos IA de inmediato porque `AI_Credit_System.md` ya define explícitamente que un upgrade/downgrade aplica desde el siguiente ciclo, no al instante — conectar un handler reactivo ahí contradiría esa regla ya aprobada; `staff_trasladado` no tiene ninguna función real de "trasladar Staff entre Sedes" todavía en el producto. Fabricar handlers vacíos para estos cuatro habría sido un "botón muerto" (Regla de Oro) disfrazado de arquitectura.
