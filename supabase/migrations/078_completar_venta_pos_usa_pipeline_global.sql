-- StylerNow — Migración 078: `completar_venta_pos()` llama al orquestador
-- genérico `ejecutar_pipeline_evento('venta_completada', ...)` en vez del
-- nombre específico `ejecutar_pipeline_venta_completada()` (eliminado en
-- la migración 077). Único cambio real: la última línea antes de
-- retornar. Ningún otro comportamiento de la fase crítica se toca.
...
begin
  ...
  -- Pipeline de Eventos GLOBAL (ADR-014, Fase F) — ya no un nombre
  -- específico de venta, sino el mismo bus que sirve cualquier evento
  -- futuro (reserva_confirmada, cliente_registrado, plan_actualizado,
  -- staff_trasladado — ver migración 077).
  v_resultado_pipeline := public.ejecutar_pipeline_evento('venta_completada', v_evento);
