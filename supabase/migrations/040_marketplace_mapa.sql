-- StylerNow — Migración 040: Fase 5.3 — Marketplace: Mapa visual (MapLibre
-- + OpenStreetMap)
-- Fuente: 02-UX/04_Marketplace.md ("Mapa visual con pines de Negocios
-- cercanos — vista opcional, alternable con vista de lista"),
-- 00_MASTER_TASKLIST.md línea 36 (decisión ya tomada: MapLibre + OSM).
--
-- El Home necesita la latitud/longitud de la Sede de cada resultado para
-- dibujar los pines — marketplace_buscar() no las devolvía. Postgres NO
-- permite `create or replace function` para cambiar el RETURNS TABLE de
-- una función existente ("cannot change return type of existing
-- function") — a diferencia de agregar un parámetro nuevo (que crea una
-- sobrecarga silenciosa, ADL-020/ADL-021), esto directamente falla con un
-- error explícito. Se corrige con `drop function` seguido de `create
-- function`, nunca editando en sitio.

drop function if exists public.marketplace_buscar(text, text, text, text, int, int, double precision, double precision);

create function public.marketplace_buscar(
...
begin
  ...
  con_sede as (
    select
      ...
      sede.distancia_km,
      sede.latitud as sede_latitud,
      sede.longitud as sede_longitud,
      coalesce(sede.staff_total, 0) as staff_total,
      coalesce(sede.staff_experto, 0) as staff_experto
    from base b
    left join lateral (
      select
        s.id, s.zona_horaria, s.latitud, s.longitud,
  ...
  select
    ...
    f.sede_latitud, f.sede_longitud
  from final f
  order by
