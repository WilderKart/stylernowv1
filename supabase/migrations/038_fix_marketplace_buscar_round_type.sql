-- StylerNow — Migración 038: corrige "function round(double precision, integer) does not exist" en marketplace_buscar()
--
-- Bug real: score se computa mezclando numeric (rating_normalizado,
-- conversion_normalizada, calidad_de_staff) con double precision
-- (proximidad_normalizada/disponibilidad_normalizada, que derivan de
-- funciones trigonométricas y extract(epoch from ...)). Postgres resuelve
-- la suma como double precision, y round(double precision, integer) no
-- existe como función nativa — solo round(numeric, integer). Se corrige
-- casteando explícitamente en el ORDER BY: round(f.score::numeric, 3).
