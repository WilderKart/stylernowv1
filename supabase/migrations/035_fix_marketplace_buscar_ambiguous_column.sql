-- StylerNow — Migración 035: corrige "column reference calificacion is
-- ambiguous" en marketplace_buscar() (migración 033)
--
-- Bug real encontrado de inmediato al probar tras la migración 034: la
-- función declara `returns table (..., calificacion numeric, ...)`, y
-- PL/pgSQL registra cada columna de ese RETURNS TABLE como una variable
-- implícita visible en todo el cuerpo de la función. La línea
-- `select coalesce(avg(calificacion), 4) into v_promedio_plataforma from
-- public.resena where estado = 'VISIBLE'` referenciaba `calificacion` (y
-- `estado`) sin calificar con el alias de la tabla — Postgres no podía
-- decidir si `calificacion` era la columna de `resena` o la variable de
-- salida de la función, y lo rechaza como ambiguo (comportamiento por
-- defecto de `plpgsql.variable_conflict = error`, una protección real,
-- no un capricho). Se corrige calificando explícitamente `resena.
-- calificacion` y `resena.estado` — nunca dejar una columna sin alias en
-- una función cuyo RETURNS TABLE comparte nombres con columnas reales.
