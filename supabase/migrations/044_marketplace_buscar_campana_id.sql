-- StylerNow — Migración 044: marketplace_buscar() expone campana_id
-- Fuente: 08-Growth-Monetization/06_Advertising_System.md (Módulo 6.2).
--
-- El frontend necesita saber QUÉ campaña respalda un resultado
-- patrocinado para poder llamar registrar_clic_patrocinado()/
-- registrar_impresion_patrocinada() con el id correcto — antes
-- `patrocinado` era solo un booleano. Mismo motivo que la migración 040:
-- agregar una columna al RETURNS TABLE exige `drop function` + `create
-- function`, `create or replace` lo rechaza con un error explícito.
...
begin
  ...
  with base as (
    select
      ...
      (
        select c.id from public.campana_publicitaria c
  ...
  puntuado as (
    select
      ...
      case when cd.campana_id is not null then 1 else 0 end as patrocinio_normalizado
  ...
  select
    ...
    f.sede_id, f.proxima_disponibilidad, f.campana_id is not null, f.campana_id,
