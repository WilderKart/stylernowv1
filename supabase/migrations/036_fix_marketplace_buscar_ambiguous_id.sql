-- StylerNow — Migración 036: segunda ronda del mismo bug de la migración 035
--
-- Quedó una segunda columna sin calificar: `(select id from public.
-- temporada order by fecha_inicio desc limit 1)` dentro del cálculo de
-- Calidad_de_Staff — mismo problema exacto (`id` colisiona con la columna
-- de salida `id` del RETURNS TABLE). Se corrige calificando como
-- `temporada.id`. Se revisó el resto del cuerpo de la función en busca de
-- cualquier otra columna de salida sin calificar antes de aplicar esto,
-- para no necesitar una tercera ronda.
