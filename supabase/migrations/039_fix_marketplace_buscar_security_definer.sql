-- StylerNow — Migración 039: marketplace_buscar() debe ser SECURITY
-- DEFINER — bug real y grave encontrado por la prueba end-to-end contra
-- el cliente anon real (no service_role)
--
-- marketplace_buscar() se declaró (migración 033) SIN `security definer`
-- — corre como SECURITY INVOKER, con los privilegios y la RLS del propio
-- Cliente que llama (normalmente `anon`, un visitante del Marketplace sin
-- sesión). Tres de los seis componentes del Score leen tablas que NUNCA
-- tuvieron una política de lectura pública:
--   - `campana_publicitaria` (Patrocinio_normalizado): solo
--     `campana_select_interno` (tiene_acceso_interno) — un visitante
--     anónimo nunca ve ninguna fila, así que `exists(...)` siempre daba
--     falso sin importar si la campaña estaba realmente ACTIVA.
--   - `vinculo_staff_negocio` / `nivel_staff_consolidado`
--     (Calidad_de_Staff): mismo problema — `staff_total`/`staff_experto`
--     siempre calculaban 0 para un visitante anónimo.
--   - `reserva` (Conversión_normalizada, completadas_90d): mismo
--     problema — la prueba automatizada de este componente pasó por
--     COINCIDENCIA (el desempate por antigüedad `created_at asc`
--     favorecía al mismo Negocio que se esperaba que ganara por
--     conversión real), no porque el componente funcionara — se corrigió
--     también el diseño de esa prueba, no solo el código.
--
-- Se corrige agregando `security definer` a la función — mismo patrón ya
-- usado en el resto del proyecto (`reportes_ranking_staff`,
-- `dashboard_ranking_staff_semana`, etc.) para RPCs públicas que deben
-- calcular sobre datos internos sin depender de que el llamador tenga
-- acceso RLS directo a cada tabla subyacente. El único dato que la
-- función expone hacia afuera sigue siendo exactamente el mismo (columnas
-- ya públicas: id/nombre/slug/etc. y el booleano `patrocinado`) — no se
-- abre ninguna tabla nueva al Cliente, solo se le permite a la propia
-- función leerlas internamente para calcular el Score.
